...
var EmailReplyParser = {
	...
	parse_reply: function (text, include_signatures) {
		...
		const decodedStripedHtml = this.decodeEntities(stripedHtml);
		return this.read(decodedStripedHtml).visible_text(include_signatures);
	},

	decodeEntities: function (encodedString) {
		const translate_re = /&(nbsp|amp|quot|lt|gt|uuml|auml|ouml|szlig|Uuml|Auml|Ouml);/g;
		const translate = {
			"nbsp":" ",
			"amp" : "&",
			"quot": "\"",
  ...
  // Splits the given text into a list of Fragments.  This is roughly done by
	...
	read: function(text) {
		...
		const quoteHeadersRegex = [
			...
			/^\s*(Am\s.+\sum\s.+\sschrieb\s.+\s?.+\s?.+\s?:)$/mg, // Am DATE um TIME schrieb NAME:
			/^\s*(Am\s.+\s)schrieb.+\s?(\[|<).+(\]|>):$/m, // Am DATE schrieb NAME <EMAIL>:
			/^\s*(Am\s.+\s)schrieb.+\s?(\[|<).+\s+(\]|>):$/m, // Am DATE schrieb NAME <EMAIL>:
			/\s*(Am\s.+\s+?.+\shat\s.+\s+Folgendes\sgeschrieben\s?:)/m, // yahoo de
			/^>\s.+\shat\sam\s.+\sum\s.+\sgeschrieben:$/m, // magenta de
			/^(‐{3,}\sOriginal\sMessage\s‐{3,})$/,
			/^\sSent with\s.+\sEmail.$/,
		...
		quoteHeadersRegex.forEach((regex) => {
			...
			if (matches) {
			  ...
			  const splitText = text.split(matches[0]); // remove anything below
			  text = splitText[0];
			}
		});
		for(let i = 0; i < umlReplace.length; i++) {
			text.replace(umlReplace[i], umlReplaceTo[i]);
		}
