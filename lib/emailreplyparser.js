...
Email.prototype = {
  ...
  // Splits the given text into a list of Fragments.  This is roughly done by
	...
	read: function(text) {
		...
		var enPatt = /^(On\s*.*\s*.*wrote:)$/im;
		var frPatt = /^(Le\s*.*\s*.*a écrit:)$/im;
		var esPatt = /^(El\s*.*\s*.*escribió:)$/im;

		if(enPatt.test(text)) {
			var reply_header = (enPatt.exec(text))[0];
			text = text.replace(reply_header, reply_header.replace(/\s/g, ' '));
		}

		if(frPatt.test(text)) {
			var reply_header = (frPatt.exec(text))[0];
			text = text.replace(reply_header, reply_header.replace(/\s/g, ' '));
		}

		if(esPatt.test(text)) {
			var reply_header = (esPatt.exec(text))[0];
			text = text.replace(reply_header, reply_header.replace(/\s/g, ' '));
		}
	...
	scan_line: function(line) {
		//line = line.replace(/\u2014/g, "-");
		var SIG_REGEX = new RegExp('^(\\s*\u2014+\\s*$|\\s*-+\\s*$|_+\\s*$|\\w-$|\\w_$)|(^(\\w+\\s*){1,3} ' + ("Sent from my").reverse() + '|' + ("Envoyé de mon").reverse() + '$)');
...
module.exports.BreezyEmailReplyParser = BreezyEmailReplyParser;
